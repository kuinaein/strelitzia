...
export default extendVue({
  data () {
    return {
      ...
      editing: {
        enabled: true,
        postDate: 1,
        nextPostDate: moment().add(1, 'month').date(1).format(MOMENT_ISO_DATE_FORMAT),
        remarks: '',
        debitAccountId: null,
  ...
  methods: {
    ...
    createSchedule () {
      ...
      this.editing.debitAccountId = this[AccountModule.stateKey.accountTitles][0].id;
      this.editing.creditAccountId = this[AccountModule.stateKey.accountTitles][1].id;
    ...
    doSave () {
      const promise = this.editing.id
        ? axios.put(`${this.apiRoot}/journal/schedule/${this.editing.id}`, this.editing)
    ...
    doJournalAll () {
      ...
      this.doJournalImpl(this.overdues).then(savedIds => {
    ...
    doJournal (schedule) {
      if (!confirm(`「${schedule.remarks}」を「${schedule.nextPostDate}」付で記帳してよろしいですか？`)) {
        return;
      }
      this.doJournalImpl([schedule]).then(savedIds => {
